...
- Backend
  ...
  - Support `category` filtering and `sort=date_desc` on list requests.
- Frontend
  - Scaffold a React + Vite app in TypeScript.
  - Build a simple expense form, category filter, newest-first sort control, expense list/table, and a visible total for the filtered list.
...
- Deployment
  ...
  - Ensure the frontend can be deployed directly to Vercel.
  - Ensure the Go backend is structured in a Vercel-compatible way or a clearly documented Vercel-friendly deployment path.
- Reliability
